# Neko Tutorial
This is a tutorial for the [Neko](http://nekovm.org) language.

# Index
[Overview](md/overview.md)

- [Accessing Commandline Arguments](md/cmdargs.md)
- [Arrays](md/arrays.md)
- [Basics](md/basics.md)
- [Booleans](md/booleans.md)
- [Built-in Functions](md/builtins.md)
- [Commandline Tools](md/tools.md)
- [Compiling](md/compiling.md)
- [Control Flow Constructs](md/control.md)
- [Debugging in Neko](md/debug.md)
- [Frequently Asked Questions](md/faq.md)
- [Functions](md/functions.md)
- [Hashtables](md/hashtables.md)
- [Installing Neko](md/install.md)
- [Interfacing with C](md/ffi.md)
- [Neko Console](md/console.md)
- [Numbers](md/numbers.md)
- [Objects](md/objects.md)
- [Strings](md/strings.md)
- [Using Libraries](md/libs.md)
- [Using Modules](md/modules.md)
- [Walkthrough](md/walk.md)

